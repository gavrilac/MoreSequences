...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    expected = [rg.Point(425.0, 50.0),
                rg.Point(419.2, 66.1),
                rg.Point(404.3, 74.6),
                rg.Point(387.5, 71.7),
                rg.Point(376.5, 58.6),
                rg.Point(376.5, 41.4)]
    circle = rg.Circle(rg.Point(400, 50), 25)
    answer = generate_points_on_circle(circle, 9)
...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    title = 'DRAW_POINTS_ON_CIRCLE test 4 : 6 blue dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg. Circle (rg.Point(300, 300), 60)
    draw_points_on_circle(window, circle, 6, 'blue')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(seq)):
        point = rg.Point (seq[k].x , seq[k].y)
...
def run_test_pizza():
    ...
    circle = rg.Circle(rg.Point(150, 300), 100)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    circle.attach_to(window)
    seq = generate_points_on_circle(circle, number_of_slices)

    if len(seq) % 2 == 0:
        for k in range(len(seq)//2):
            line = rg.Line(seq[k], seq[len(seq)//2+k])
            line.color = color
            line.thickness = thickness
            line. attach_to(window)
    else:
        for k in range(len(seq)):
            line = rg.Line(seq[k], circle.center)
            line.color = color
            line.Thickness = thickness
            line.attach_to(window)
...
def run_test_polygon():
    ...
    window = rg. RoseWindow(550 , 400, title)
    circle = rg. Circle(rg.Point(150, 150), 80)

    circle.outline_thickness = 5
    polygon(window, circle, 7 , 'green', 3)
    window.close_on_mouse_click()
...
q
    ...
    seq = generate_points_on_circle(circle, number_of_segments)
    for k in range(len(seq)):
        line = rg.Line(seq[k - 1], seq[k])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
...
def run_test_fancy_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'Fancy Polygon test 4: 20 segments, hops = 5.'
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'red'
    fancy_polygon(window, circle, 25, 5, 'green', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(len(seq)):
        line = rg.Line(seq[k-len(seq) // 2-1], seq[k-len(seq) // 2-1 +hops_to_next_point])
        line.arrow = 'last'
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
